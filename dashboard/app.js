...
function disclosureHtml(record) {
  ...
  return `
    <div class="report-head">
      ...
      <p class="report-stamp">${record.site} ${originBadgeMarkup(record)}</p>
    </div>
    ...
    <div class="origin-disclosure ${origin.className}">
      ...
      <code>source=${escapeHtml(origin.source)}</code>
      <code>mode=${escapeHtml(origin.mode)}</code>
      <code>adapter=${escapeHtml(origin.adapter)}</code>
    </div>
...
function classifyBountyOrigin(record = {}) {
  const metadata = record.metadata || {};
  const source = metadata.source || record.source || (record.appRunMode === APP_MODE.SIMULATION ? "dashboard-sim" : "unknown");
  const mode = metadata.app_mode || record.appRunMode || appRunMode || "unknown";
  const adapter = metadata.intake_adapter || record.intakeAdapter || "";
  ...
  if (normalizedSource === "dashboard-sim" || (record.id && String(record.id).startsWith("B-") && !adapter)) {
    return {
      label: "Simulation",
      source: sourceKey,
      mode,
      adapter: adapter || "--",
      isRealScrape: false,
      className: "origin-sim",
      description: "Generated by dashboard simulation. Do not treat as real bounty data."
    };
  }

  if (normalizedSource === "manual_fixture") {
    return {
      label: "Fixture",
      source: sourceKey,
      mode,
      adapter: adapter || "real_scrape_preflight",
      isRealScrape: false,
      className: "origin-fixture",
      description: "Shadow Real pipeline fixture. Real write path, not a real scraped bounty."
    };
  }

  if (normalizedAdapter === "real_scrape_preflight" && normalizedSource !== "unknown") {
    return {
      label: "Real Scrape",
      source: sourceKey,
      mode,
      adapter,
      isRealScrape: true,
      className: "origin-real",
      description: "Ingested from a real scrape adapter through the preflight pipeline."
    };
  }
  ...
  return {
    ...
    description: "Origin metadata is incomplete. Review before using in production reports."
  };
}
...
function buildReportDataset() {
  ...
  const originCounts = bountyRecords.reduce(
    (counts, record) => {
      const origin = classifyBountyOrigin(record);
  ...
  return {
    ...
    rows: bountyRecords.map((r) => {
      const origin = classifyBountyOrigin(r);
      return {
        id: r.id,
        origin: origin.label,
        isRealScrape: origin.isRealScrape ? "Yes" : "No",
...
function reportToSheetRows(reportData) {
  ...
  const rows = [
    ...
    [],
    ["ID", "Origin", "Is Real Scrape", "Source", "Mode", "Adapter", "Site", "Site URL", "Type", "Price USD", "Stage", "Won", "Overdue", "Due Date"]
  ];
...
function applyReportWorkbookLayout(sheet, reportData) {
  ...
  sheet["!merges"] = [
    ...
    { s: { r: 4, c: 4 }, e: { r: 7, c: 13 } },
    { s: { r: 9, c: 4 }, e: { r: 11, c: 5 } },
    { s: { r: 9, c: 7 }, e: { r: 11, c: 13 } }
  ];
  sheet["!autofilter"] = { ref: `A14:N${Math.max(lastRow, 14)}` };
  sheet["!freeze"] = { xSplit: 0, ySplit: 14, topLeftCell: "A15", activePane: "bottomLeft", state: "frozen" };
  sheet["!ref"] = `A1:N${Math.max(lastRow, 16)}`;
  ...
  styleRange(4, 4, 5, 14, headerStyle);
  ...
  for (let rowNumber = 15; rowNumber <= lastRow; rowNumber += 1) {
    ...
    const priceCell = sheet[`J${rowNumber}`];
    if (priceCell) {
      priceCell.z = "$#,##0";
      priceCell.s = mergeStyle(priceCell.s, { alignment: { horizontal: "center", vertical: "center", wrapText: true } });
    }

    const originValue = String(sheet[`B${rowNumber}`]?.v || "").toLowerCase();
    const originFill = {
      simulation: palette.gray,
      fixture: palette.orange,
      "real scrape": palette.lime,
      unverified: palette.red
    }[originValue];
    if (originFill) {
      styleRange(rowNumber, rowNumber, 2, 6, {
        fill: { fgColor: rgb(originFill) },
        alignment: { horizontal: "center", vertical: "center", wrapText: true },
        font: { bold: true }
      });
    }

    const stageValue = String(sheet[`K${rowNumber}`]?.v || "").toLowerCase();
    const stageFill = {
    ...
    const wonValue = String(sheet[`L${rowNumber}`]?.v || "").toLowerCase();
    if (wonValue === "yes") {
      applyStyle(rowNumber, 12, {
        fill: { fgColor: rgb(palette.lime) },
        font: { bold: true, color: rgb("245A12") },
        alignment: { horizontal: "center", vertical: "center", wrapText: true }
      });
    } else {
      applyStyle(rowNumber, 12, { alignment: { horizontal: "center", vertical: "center", wrapText: true } });
    }

    const overdueValue = String(sheet[`M${rowNumber}`]?.v || "").toLowerCase();
    if (overdueValue === "yes") {
      applyStyle(rowNumber, 13, {
    ...
    applyStyle(rowNumber, 8, { font: { color: rgb("0563C1"), underline: true } });
    applyStyle(rowNumber, 11, { alignment: { horizontal: "center", vertical: "center", wrapText: true } });
    applyStyle(rowNumber, 14, { alignment: { horizontal: "center", vertical: "center", wrapText: true } });
  }

  for (let rowNumber = 15; rowNumber <= lastRow; rowNumber += 1) {
    const urlCell = sheet[`H${rowNumber}`];
    if (urlCell?.v) {
  ...
  const rows = generatedReportData.rows
    .map((r) => {
      return `
        <tr>
          <td>${r.id}</td>
          <td><span class="origin-badge ${originClassForLabel(r.origin)}">${r.origin}</span></td>
          <td>${r.isRealScrape}</td>
          <td>${r.source}</td>
    ...
    .join("");
  const originCounts = generatedReportData.kpis.originCounts;
...
function originClassForLabel(label = "") {
  return {
    simulation: "origin-sim",
    fixture: "origin-fixture",
    "real scrape": "origin-real",
    unverified: "origin-unknown"
  }[String(label).toLowerCase()] || "origin-unknown";
}
